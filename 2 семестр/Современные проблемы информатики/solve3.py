...
"""
1. Что такое многозадачный (мультизадачный) режим работы?

   Многозадачный режим – способность ОС обеспечивать параллельное (или
   псевдопараллельное) выполнение нескольких задач. В нашей модели это
   симулируется циклом, где каждый квант времени может быть отдан одному
   процессу (при num_cores=1) или нескольким (при num_cores>1).

2. Как осуществляется мультизадачный режим на однопроцессорном компьютере?

   На однопроцессорном компьютере многозадачность реализуется быстрым
   переключением процессора между задачами. В программе это отражено
   последовательным выполнением выбранного процесса внутри одного шага
   (цикл for _ in range(steps) с выбором одного процесса через Weights).

3. Перечислить типы мультизадачности, реализуемые в операционных системах.

   Типы многозадачности:
   - невытесняющая (простое переключение, фоновые задачи заморожены);
   - кооперативная (задачи сами отдают управление);
   - вытесняющая (ОС принудительно переключает задачи по прерываниям/таймеру).
   В нашей модели реализована вытесняющая многозадачность с элементами
   кооперативной? Нет, передача управления принудительная и случайная,
   процессы не могут повлиять на переключение – это вытесняющая схема.

4. Что такое приоритет задачи (процесса)?

   Приоритет задачи – целое число, определяющее относительную долю
   процессорного времени (вероятность получения кванта). В коде приоритеты
   задаются в конструкторах Reader/Writer и используются классом Weights,
   который строит pool повторением индексов пропорционально приоритетам.

5. Каким образом устанавливаются приоритеты?

   Приоритеты устанавливаются либо пользователем, либо системой на основе
   политик (например, интерактивные vs фоновые). В нашем эксперименте
   приоритеты заданы жёстко: читатели имеют фиксированные приоритеты
   (READER_PRIORITIES = (1,3,2)), а приоритет писателя варьируется.

6. Какие задачи имеют наивысший приоритет?

   Наивысший приоритет обычно имеют системные критические процессы
   (обработчики прерываний, планировщик). В модели наивысший приоритет
   может быть у писателя (когда writer_priority велико), но это лишь
   условность эксперимента.

7. В чем отличие корпоративной многозадачности от вытесняющей? 

   Отличие кооперативной многозадачности от вытесняющей:
   - кооперативная: процесс сам решает, когда отдать управление (например,
     вызов yield). Нет защиты от зависания одного процесса.
   - вытесняющая: ОС принудительно отбирает управление (по таймеру или
     прерываниям), гарантируя справедливое распределение.
   В нашей модели переключение происходит случайным образом в каждом кванте
   независимо от состояния процессов – это вытесняющая схема.

8. Какая мультизадачность реализована в разработанной в работе модели. 

   В разработанной модели реализована вытесняющая многозадачность с
   приоритетным вероятностным планированием. На каждом шаге (кванте)
   выбор процесса осуществляется случайно, с вероятностью, пропорциональной
   приоритету. Расширение на многоядерность позволяет за один квант
   выполнять до num_cores читателей одновременно, но писатель всегда
   монополизирует доступ, если выбран.

9. Что такое язык программирования реального времени?

   Язык программирования реального времени – язык, в среду исполнения
   которого встроен планировщик задач (например, обработчик прерываний
   таймера), позволяющий организовать многозадачность без поддержки ОС.
   Примеры: Modula-2, Ada. В нашей работе мы не используем такой язык,
   но сама идея встроенного планировщика напоминает нашу функцию simulate
   с бесконечным циклом и переключением процессов.
"""
