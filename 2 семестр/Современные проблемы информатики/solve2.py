...
"""
1. Суть метода полного перебора.
   Полный перебор (брутфорс) заключается в генерации всех возможных вариантов
   решения (в задаче коммивояжёра — всех возможных маршрутов), вычислении
   целевой функции для каждого варианта и выборе наилучшего. Гарантирует
   нахождение глобального оптимума за счёт исчерпывающего просмотра.

2. Преимущества и недостатки полного перебора.
   Преимущества:
     - Гарантирует точное оптимальное решение.
     - Прост в реализации, не требует эвристик.
     - Применим к любым типам целевых функций.
   Недостатки:
     - Экспоненциальный/факториальный рост числа вариантов (O(n!) для TSP).
     - Практически неприменим при n > 15–20 из-за колоссального времени счёта.
     - Требует много памяти, если варианты хранятся явно.

3. Оптимизация полного перебора методом ветвей и границ.
   Метод ветвей и границ сокращает перебор путём отсечения заведомо
   бесперспективных подмножеств решений. Строится дерево вариантов (ветвление).
   Для каждого узла вычисляется нижняя граница стоимости любого маршрута,
   продолжающего данный частичный путь. Если эта граница больше или равна
   стоимости уже найденного наилучшего решения (рекорда), всё поддерево
   отбрасывается (граница). Таким образом, явно просматривается лишь
   небольшая часть всех возможных маршрутов.

4. Методическая и практическая значимость задачи коммивояжёра.
   - Методическая: задача коммивояжёра — классический пример NP-трудной
     проблемы, на которой изучают и сравнивают точные и приближённые методы
     дискретной оптимизации, теорию сложности и приёмы борьбы с
     комбинаторным взрывом.
   - Практическая: возникает в логистике (построение маршрутов доставки),
     проектировании печатных плат (сверление отверстий), планировании
     производства, секвенировании генома и многих других областях, где
     требуется найти оптимальную последовательность обхода точек.

Вопрос 5-9 по методу Джонсону, т.е. не по варианту нашей группы
"""
